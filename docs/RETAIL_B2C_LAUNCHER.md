...
This launcher is the stable LAN entrypoint for the Retail B2C block.
...
```text
http://<auto-detected-LAN-IP>:18810/index.html
```
...
The launcher binds the server to `0.0.0.0` and auto-detects a LAN IPv4 address for the browser URL. It does not use `127.0.0.1` or `localhost` as the working URL.

The launcher uses port `18810` because local `8790` was previously observed as unstable or occupied by another listener. MESER deployment can still use:
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "D:\Codex\CRM\retail-crm\launch-retail-crm.ps1" -HostName 0.0.0.0 -Port 18811 -NoBrowser
```
